...
**lexer・parserは実装済みです。型推論・評価器は未実装です。** 型定義、関数の入口、練習用テスト、Wasmとの接続、Web画面、GitHub Pages用ワークフローを用意しています。サンプルの結果は、各段階を実装すると表示されるようになります。
...
表示されたローカルURLを開き、「実行する」を押してください。Token・AST欄に解析結果が表示されます。型推論・評価器は未実装です。
